...
function OrderStatus() {
  ...
  async function goToLinePay() {
    try {
      let getResponse = await axios.get(
        `http://localhost:3001/api/linePay/checkout/${order_id}`,
        {
          withCredentials: true,
        }
      );
      console.log('linePayGetResponse', getResponse.data);

      let response = await axios.post(
        `http://localhost:3001/api/linePay/linePay/${order_id}`,
        {
          withCredentials: true,
        }
      );
      console.log('linePay', response.data.redirectUrl);
  ...
  return (
    ...
    >
      ...
      <Col>
              ...
              return (
                ...
                >
                  <Col className={`col-7 pe-0`}>
                    ...
                    >
                      <Col
                        className={`${style.orderItemImg} col-3 p-0 d-none d-md-block `}
                      >
                        <img
                ...
                <tr>
                  ...
                  <td>
                    {orderMemberInfo.pay === 1 ? (
                      ...
                      '銀行轉帳'
                    ) : orderMemberInfo.pay === 4 ? (
                      <Image
                        src={require('../../Img/line_pay.png')}
                        style={{ width: '60px' }}
                      />
                    ) : (
                      '付款失敗'
                    ...
                    {orderMemberInfo.pay === 4 &&
                    orderMemberInfo.pay_info.length <= 0
                      ? ' 尚未付款'
                      : '已確認付款'}
                  </td>
                  <th>訂單備註</th>
        ...
        <Row
          className={`${style.bgYellow} chicofgo_gray mb-4 align-items-center py-2 `}
        >
          {orderMemberInfo.pay_info ? (
            ''
          ) : (
            <Col className={`${style.bgYellow} px-4 `}>
              <Button variant="chicofgo-brown" onClick={goToLinePay}>
                前往付款
              </Button>
            </Col>
          )}

          <Col className={`${style.bgYellow} text-end`}>
            {/* 內容4 */}
            <span className={`${style.priceTotal} `}>
